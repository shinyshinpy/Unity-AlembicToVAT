...
public class ATV_Editor : EditorWindow
{
		...
		if (alembic != null)
		{
...

			{
				...
				for (int frame = 0; frame < framesCount; frame++)
				{
					...
					int triangleCount = TriangleCountFromChildren(MeshToBake);
	...
	private int TriangleCountFromChildren(Transform target)
	{
		int triangleCount = 0;
		...
		for (int i = 0; i < target.childCount; i++)
		{
			...
			{
				triangleCount = TriangleCountFromChildren(child);
				if(triangleCount != 0) return triangleCount;
			}

			MeshFilter localMeshFilter = child.GetComponent<MeshFilter>();
	...
	private bool SearchAndAnalyzeMeshFilterDatas(
		Transform target,
		ref MeshFilter meshFilter,
		ref int verticesCount,
		ref int trianglesIndexCount,
		ref bool hasNormal,
		ref bool hasUVs,
		ref bool hasColors
	)
	{
		for (int i = 0; i < target.childCount; i++)
		{
			...
			if(child.childCount > 0)
			{
				bool result = SearchAndAnalyzeMeshFilterDatas(
					child, 
					ref meshFilter,
					ref verticesCount, 
					ref trianglesIndexCount, 
					ref hasNormal, 
					ref hasUVs, 
					ref hasColors);
			...
			Debug.Log("find MeshFilter");
			verticesCount += meshFilter.sharedMesh.vertices.Length;
			trianglesIndexCount += meshFilter.sharedMesh.triangles.Length;

			hasNormal |= (meshFilter.sharedMesh.normals.Length > 0);
			hasColors |= (meshFilter.sharedMesh.colors.Length > 0);
			hasUVs |= (meshFilter.sharedMesh.uv.Length > 0);
	...
	IEnumerator ExportFrames()
	{
		...
		if((MeshToBake == null) || (MeshToBake.childCount <= 0)) yield break;
		...
		{
			SearchAndAnalyzeMeshFilterDatas(
				MeshToBake, 
				ref meshFilter,
				ref verticesCount, 
				ref trianglesIndexCount, 
		...
		if(meshFilter == null)
		{
			Debug.LogError("can't find MeshFilter");
			yield break;
		}
		...
		{
			MeshFilter localMeshFilter = meshFilter;
			if (localMeshFilter != null)
			{
		...
		if (resPosTexture == null)
			Debug.LogWarning("Can't load position texture " + finalExportPath + ExportFilename + "_position.asset");
		if (resNormalTexture == null)
			Debug.LogWarning("Can't load position texture " + finalExportPath + ExportFilename + "_normal.asset");
